...
(async () => {
  ...
  try {
    ...
    // const browser = await chromium.connectOverCDP("http://browser-box:9223");
    const response = await fetch("http://browser-box:9223/json");
    const data = await response.json();

    const wsUrl = data[0].webSocketDebuggerUrl.replace("localhost", "browser-box");
